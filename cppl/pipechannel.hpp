...
{	
	...
	class PipeChannel : public BufferProcessor
	{
		public:
			...
			#ifdef LINUX
				PipeChannel(int sockt, std::function<CPPL::AbstractServerReceiver* (void)> createAbstractServerReceiverProc, unsigned int timRX, unsigned int timTX);
			#endif

			#ifdef WINDOWS
				PipeChannel(HANDLE handlePipe, std::function<CPPL::AbstractServerReceiver* (void)> createAbstractServerReceiverProc, unsigned int timRX, unsigned int timTX);
			#endif
